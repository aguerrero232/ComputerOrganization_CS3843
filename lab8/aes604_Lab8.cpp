// Lab8.cpp
//
// This program is used for lab #8
//

#include <windows.h>
#include <stdio.h>

//#define STRLEN_TEST

char n[] = "Exams are fun!";
char q[] = "  And this lab sux!";

int FUNC1_Solution(char *par1, int *par2);
int FUNC2_Solution(char *par1, int *par2);
int FUNC3_Solution(char *par1, int *par2, char X);

int FUNC2_Student(char *par1, int *par2);

int FUNC3_Student(char *par1, int *par2, char X)
{
	int len = 0;
	FUNC2_Student(par1, &len);
	__asm
	{
			push edi
			push ecx 
			push eax
			sub ecx, ecx
			mov ecx, len 
			mov al, X
			mov edi, par1
			std 
			sub eax, eax
			repne scas
			cld
			lea eax, [ecx + 1]
			mov edi, [ebp + 12]
			mov[edi], eax
			pop eax			
			pop ecx
			pop edi
	}
	return 0;
} // FUNC3_Student


// scans a string for the NULL terminator, returns length in par2
// This function MUST make use of some string instructions
int FUNC2_Student(char *par1, int *par2)
{
	__asm
	{
		push edi 
		push ecx 
		push eax
		sub ecx, ecx
		mov edi, [ebp + 8]
		not ecx
		sub al, al
		cld
		repne scasb
		not ecx
		lea eax, [ecx-1]
		mov edi, [ebp+12]
		mov [edi], eax
		pop eax
		pop ecx 
		pop edi
	}

	return 0;
} // FUNC2_Student

// scans a string for the NULL terminator, returns length in par2
int FUNC1_Student(char *par1, int *par2)
{
	__asm
	{
		push edi
		push ecx
		push eax 
		mov edx, 0
		mov ecx, 0 
		mov eax, dword ptr[par1]
		countLoop:
			mov dl, byte ptr[eax+ecx]
			cmp dl,0
			je done
			inc ecx
			jmp countLoop
		done:
			mov edi, [ebp + 12]
			mov[edi], ecx
		pop eax
		pop ecx
		pop edi

	}

	return 0;
} // FUNC1

int FUNC0(DWORD *funcAddress)
{
	int len = 0;

	__asm
	{
			lea ecx, len		// pushing address of len, in C would be func( &len );
			push ecx
			lea ecx, n			// pushing address of n, a global array
			push ecx
			call funcAddress
			add esp, 8

#ifdef STRLEN_TEST
			mov len, eax		// strlen returns length in eax, FUNC1/FUNC2 return it in the parameter
#endif 
	}

	printf("FUNC: Length = %d\n", len);
	return len;
} // FUNC0


int main(int argc, char *argv[])
{
	char searchChar = '!';
	char *ptrNewString;
	int phraseLength;

#ifdef STRLEN_TEST

	phraseLength = FUNC0( (DWORD *)  strlen );

#else

//	phraseLength = FUNC0( (DWORD *) FUNC1_Solution);	// you can uncomment this if you like

//	phraseLength = FUNC0( (DWORD *) FUNC2_Solution);	// I'll provide these later

/*	FUNC3_Solution(n, &phraseLength, searchChar);
	if(phraseLength > 0)
		printf("Found %c at index %d in n=%s.\n\n", searchChar, phraseLength, n);
	else
		printf("Did NOT find %c in n=%s.\n\n", searchChar, n);
*/
	phraseLength = FUNC0( (DWORD *) FUNC1_Student);

	phraseLength = FUNC0((DWORD *)FUNC2_Student);

	FUNC3_Student(n, &phraseLength, searchChar);
	
	if (phraseLength > 0)
		printf("Found %c at index %d in n=%s.\n\n", searchChar, phraseLength, n);
	else
		printf("Did NOT find %c in n=%s.\n\n", searchChar, n);
		


#endif

	// this section allocates 1024 bytes to concatenate the two strings
	ptrNewString = (char *) malloc(1024);
	if(ptrNewString == NULL)
	{
		printf("Error - Could not allocate 1024 bytes.\n\n");
		exit(-1);
	}

	__asm
	{
		mov edi,ptrNewString
		lea esi,n
		xor ah,ah

REPEAT_OP:
		lodsb
		stosb
		test al,al
		jne REPEAT_OP;

		test ah,ah
		jne END_OP;

		inc ah
		lea esi,q
		dec edi
		jmp REPEAT_OP;

END_OP:
		nop
	}

	printf("NewString: %s\n", ptrNewString);
	return 0;
} // main
